...
public partial class MainWindow : Window
{
    ...
    public MainWindow(MainViewModel viewModel, ShopListView shopListView, MessageView messageView, IShopService shopService, INavigationService navigationService, NewsView newsView)
    {
        InitializeComponent();
        DataContext = viewModel; Icon = CreateRameniaIcon(); MainContent.Content = shopListView;
        _shopListViewModel = (ShopListViewModel)shopListView.DataContext; _shopService = shopService; _navigationService = navigationService; _newsView = newsView;
        _shopListViewModel.SelectedShopChanged += ShopListViewModel_SelectedShopChanged; _shopListViewModel.ShopsChanged += ShopListViewModel_ShopsChanged;
        _newsView.RequestBack += NewsView_RequestBack; Loaded += MainWindow_Loaded; InitializeMap();
    }

    private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        if (_initialLocationRequested) return; _initialLocationRequested = true; await TryShowCurrentLocationAsync(false, false);
    }

    private void NewsButton_Click(object sender, RoutedEventArgs e)
    {
        MainArea.Visibility = Visibility.Collapsed; NewsContent.Content = _newsView; NewsContent.Visibility = Visibility.Visible; HeaderSubtitle.Text = "ラーメンの最新情報";
    }
    ...
    {
        NewsContent.Visibility = Visibility.Collapsed; NewsContent.Content = null; MainArea.Visibility = Visibility.Visible; HeaderSubtitle.Text = "ラーメン店を地図から探す";
    }
    ...
    private void MapControl_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            double resolution;
            if (_minLon == _maxLon && _minLat == _maxLat) resolution = _map.Navigator.Resolutions.Count > 12 ? _map.Navigator.Resolutions[12] : _map.Navigator.Resolutions[^1];
